...
# conn = cx_Oracle.connect(user='rushi', password='ware1234', dsn=dsn_tns) 


url="rushi/ware1234@localhost:1521/XEPDB1"
    ...
    try:
        ...
        if id is not None:
            ...
            if len(c.fetchall()) == 0:
                ...
                Label(newWindow ,text = "Update values in table:", background="green").grid(row = 0, column = 0, padx=10, pady=10)
                c.execute(f'''SELECT column_name
                            FROM USER_TAB_COLUMNS
                            WHERE table_name = '{selected_tb}'
                            ''')
